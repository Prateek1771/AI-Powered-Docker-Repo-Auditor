...
jobs:
  ...
  test-python:
    ...
    steps:
      ...
      # Not a service container either, and for a different reason: `services:`
      # takes only image/env/ports/volumes/options - there is no `command`, and
      # the official redis image takes --requirepass as an argument, not an
      # environment variable. So CI exercises the SAME authenticated Redis the
      # laptop and AWS do, rather than the one configuration nobody tests.
      - name: Start Redis
        run: |
          docker run -d --name redis -p 6379:6379 -e REDISCLI_AUTH=localdev redis:7-alpine redis-server --requirepass localdev
  ...
  #
  # Push only. This job receives OPENAI_API_KEY as TF_VAR_llm_api_key and
  # writes a tfplan containing it; running that on pull_request hands the
  # secret to any workflow run a PR can trigger. The other secret-bearing
  # jobs already gate on event_name and this one did not.
  tf-plan:
    runs-on: ubuntu-latest
  ...
  build:
    ...
    steps:
      ...
      - uses: aws-actions/configure-aws-credentials@v4
        with:
          # Likewise: the build role is the one that may push to ECR, and
          # quietly building as the deploy role hid a missing secret.
      ...
      - uses: docker/build-push-action@v6
        with:
          ...
          push: true
          # The SHA tag, and only the SHA tag. Deploy latest and you cannot
          # say which commit is running, which makes rollback guesswork -
