...
> secret, was not actually in place. S3 has done locking natively since 1.10 with
> `use_lockfile=true`, writing a `<key>.tflock` object beside the state. No second table, no
> second thing to bootstrap. The root `required_version` is now bounded at both ends, which is
...
> it. `special = false` is load-bearing: an ElastiCache `auth_token` may not contain `/`, `"`,
> `@` or spaces, so 48 alphanumeric characters clears both that rule and the 16–128 length
> range.
...
> images. It is now two task roles and three execution roles, each derived from actual call
> sites. Two grants disappeared entirely: `dynamodb:Query` on the *jobs* table, whose only
> caller is a test, and the frontend's ability to read the OpenAI key for a container that
> declares no secrets.
>
> One grant that looks dead and is not: `sqs:GetQueueAttributes` has no caller in `app/`, but it
> is the worker's **container health check**. Remove it during a least-privilege pass and the
> task never reports healthy, and ECS kills and restarts it forever.
...
> looks fixed and is not — plus the `auth_token` from the secrets module. The two are one
> control: AWS rejects an auth token without transit encryption.
...
> already `USER` down to 1001 at build time. What was missing was the task definition *saying
> so*, which is what stops an image regression silently promoting a task to root. P4-3 had
> already shown that exact failure happening to Redis.
>
> **`linuxParameters.tmpfs` is EC2-only.** On Fargate the only way to have a writable directory
> under a read-only root is a task-level `volume {}` with no configuration block — ephemeral
> task storage — plus `mountPoints`. The worker and API get one at `/tmp`; the worker genuinely
> needs it, because `TRIVY_CACHE_DIR=/tmp/trivy-cache` is baked into the image and Trivy needs
> scratch space to extract layers. These are the first volumes in the stack.
>
> Two environment variables were fixed at the same time, both of which only a read-only root
> makes obvious. `SCANNER_MODE` was never set on the **api** task, and the api image does not
> bake it, so the deployed API defaulted to `"socket"` and believed it had a Docker daemon —
> `_socket_mode_only()` never fired and `POST /images/upload` 500'd on a permission error
> instead of returning the 404 its docstring promises. And `ENRICHMENT_CACHE_DIR` defaulted to
...
> written literally without undoing the point of Secrets Manager. It becomes an `sh -c` shim
> that expands `$REDIS_PASSWORD` from the injected secret at start. Note the bare `$` — written
> `${REDIS_PASSWORD}`, Terraform would try to interpolate it and the plan fails.
>
> **That shim quietly promoted Redis to root, and the fix is the `user = "999"` line.** The
> image's `docker-entrypoint.sh` drops from root to uid 999 with `setpriv` when `$1` is
> `redis-server`; overriding `entryPoint` skips that branch entirely. Verified both ways in a
> container: without `user`, `ps` shows `root redis-server`; with it, `redis`. A hardening
> change that silently un-hardens the thing it touches is the worst kind, and in a product that
> ships a CIS *run as non-root* check it would have been quite the finding.
>
> The health check gets the same secret a second time under the name `REDISCLI_AUTH`, which
> `redis-cli` reads by itself — so the probe needs no `-a` and the token never reaches its argv,
> thirty seconds at a time. It greps for `PONG` rather than trusting the exit code, because an
> unauthenticated ping prints `NOAUTH Authentication required.` and a liveness probe should not
> depend on how `redis-cli` exits on error replies.
>
> `assign_public_ip` also stops being a hardcoded `true` and follows `local.public` like the
> other three services. That one is cosmetic — the whole resource is `count = local.public ? 1 :
> 0`, so it is only ever evaluated when that is already true — but a hardcoded exception is how
