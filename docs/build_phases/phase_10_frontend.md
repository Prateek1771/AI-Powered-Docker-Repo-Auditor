...
>
> The worst of it: `Finding` was a four-member union with no `secret`, so
> `CATEGORY_ICON[finding.category]` type-checked while returning `undefined` at runtime. One
> detected secret threw `Element type is invalid` and, with no error boundary, replaced the
> whole report with Next's error page. The scores were typed `number` where the backend returns
> `number | null`, so "we could not tell" rendered as a red 0/100.
>
> The real fix is generating this file from the OpenAPI schema rather than writing it. That is
> recorded in §17 as the long-term answer; this phase made the mirror correct again and added
> tests that fail when it is not.
